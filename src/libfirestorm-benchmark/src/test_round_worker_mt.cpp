...
namespace firestorm {
    ...
    void run_test_round_worker(const shared_ptr<spdlog::logger> &log, const shared_ptr<mapper_factory> &factory,
        ...
        auto total_duration_ms = chrono::duration_cast<chrono::milliseconds>(end_time - start_time).count();
        ...
        auto vectors_per_second = static_cast<float>(num_vectors) / ms_to_s(total_duration_ms_adjusted);
        log->info("- Processed {} vectors in {} ms ({} vectors/s, {} workers)",
                  num_vectors, total_duration_ms_adjusted, vectors_per_second, actual_worker_count);
    }
}
